...
import { parseWordDictEntry } from "./parse-word-dict-entry.js";
...
const _RcxDict = class {
  ...
  static async create(config) {
    if (!_RcxDict.instance) {
      _RcxDict.instance = new _RcxDict(config);
      await _RcxDict.instance.init();
    }
    return _RcxDict.instance;
  }
  ...
  wordSearch(word, doNames, max) {
    ...
    let p;
    const trueLen = [0];
    const entry = _RcxDict.createDefaultDictEntry();
    p = 0;
    reason = "";
  ...
  translate(text) {
    let e;
    const o = __spreadValues({ textLen: text.length }, _RcxDict.createDefaultDictEntry());
    let skip;
    while (text.length > 0) {
  ...
  makeText(entry, max) {
    ...
    if (entry.kanji) {
      ...
      result.push(entry.onkun.replace(/\.([^\u3001]+)/g, "（$1）") + "\n");
      if (entry.nanori.length) {
      ...
      for (const { code, kanjiInfoLabel } of _RcxDict.KANJI_INFO_LABELS) {
...
let RcxDict = _RcxDict;
RcxDict.KANJI_INFO_LABELS = [
